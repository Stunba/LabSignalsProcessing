...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var pointsCountSlider: UISlider!
    ...
    var selectedFile: String = "" {
        didSet {
            fileButton.setTitle("FILE: \(selectedFile)", for: .normal)
            yVals = read(from: selectedFile)
            sliderValues = sliderValues(yVals.count)
            pointsCountSlider.minimumValue = 0
            pointsCountSlider.maximumValue = Float(sliderValues.count-1)
            pointsCountSlider.value = pointsCountSlider.maximumValue
            pointsCount.text = "POINTS COUNT: \(sliderValues.last!)"
            draw(with: [Double](yVals[0..<sliderValues.last!]))
        }
    }
...
//        yAxis.axisMaximum = 1
        ...
        pointsCountSlider.isContinuous = false
        selectedFile = "PRIM1"
    }
    
    func sliderValues(_ count: Int) -> [Int] {
        var values: [Int] = []
        var value = 64
        while value <= min(count, max) {
            values.append(value)
            value *= 2
        }
        return values
    ...
    func read(from fileName: String) -> [Double] {
    ...
    @IBAction func pointsCountValueChanged(_ sender: UISlider) {
        let value = round(sender.value)
        sender.value = value
        let index = Int(value)
        let count = sliderValues[index]
        pointsCount.text = "POINTS COUNT: \(count)"
        draw(with: [Double](yVals[0..<count]))
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        switch id {
        case "file":
            let vc = segue.destination as! SelectFileTableViewController
            vc.selectFileAction = { file in
